...
const DashboardPage = (() => {
  ...
  // init() больше НЕ регистрирует роут — это делает app.js через registerPages().
  // Здесь только привязываем слушатели фильтров (один раз).
  function init() {
    bindFilters();
  }

  // render() — полный рендер: заполнить select'ы + применить фильтры.
  // Вызывается Router при navigate('dashboard').
  function render() {
    populateFilters();
    applyFilters();
  }
      ...
      ContextMenu.bind(tbody, [
        {
          ...
          action: (id) => {
            ...
            setTimeout(() => ContractsPage.openEdit(Number(id)), 80);
          }
        },
        { divider: true },
        {
  ...
  function bindFilters() {
    // Защита от повторного вызова — слушатели вешаются только один раз
    ...
    if (resetBtn) resetBtn.addEventListener('click', () => {
      ...
      ['f-contractor','f-year','f-source'].forEach(id => {
  ...
  // refresh вызывается при повторном входе на страницу (данные уже загружены,
  // но фильтры нужно пересчитать если, например, добавили контракт).
  // render вызывается при первом входе — он дополнительно перезаполняет select'ы.
