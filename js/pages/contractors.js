...
const ContractorsPage = (() => {
  ...
  function init() {
    ...
    setTimeout(() => {
      ...
      ContextMenu.bind(tbody, [
        {
          key: 'contracts', label: 'Показать контракты',
  ...
  function renderTable() {
    ...
    tbody.innerHTML = list.map(m => `<tr data-ctx data-name="${esc(m.name)}">
      ...
      <td><div style="display:grid;grid-template-columns:1fr 1fr;gap:2px 8px;font-size:10px">
  ...
  return { init, render, refresh: render, renderTable, showContracts };
})();
